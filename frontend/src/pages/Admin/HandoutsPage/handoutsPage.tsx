...
const CatalogPage = () => {
    ...
    const getHandouts = async () => {
        try {
            ...
            const formattedHandouts = course.handouts.map((handout: {title:string; file:string}, index: number) => ({
                id: index,
    ...
    const handleEditSubmit = (id: number, subject: string, file: string | File | null) => {
        if (file) {
    ...
    return (
        <div className="flex flex-col w-full">
            ...
            <div className="flex w-full justify-end">
                <button className="w-[146px] h-[32px] mr-[290px] border bg-purple-500 rounded-md"
                                ...
                                <div className="mt-3">
                                    ...
                                    <div>
                                        ...
                                        <label className="flex items-center space-x-2 cursor-pointer mt-2">
                                            ...
                                            />
                                            <div
                                                className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${uploadFile === true ? "border-purple-500" : "border-gray-400"
                                                    }`}
                                            >
                                                {uploadFile === true && (
                                        ...
                                        <label className="flex items-center space-x-2 cursor-pointer mt-2">
                                            ...
                                            />
                                            <div
                                                className={`w-6 h-6 border-2 rounded-full flex items-center justify-center ${uploadLink === true ? "border-purple-500" : "border-gray-400"
                                                    }`}
                                            >
                                                {uploadLink === true && (
                                                    <div className="w-4 h-4 bg-purple-500 rounded-full"></div>
                                            ...
                                            <div className="relative text-center">
                                                {file && (
                                                    <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-[370px] h-[37px] bg-gray-200 flex flex-row items-center px-3 shadow-md rounded-md">
                                                        ...
                                                        <button className="ml-auto text-right" onClick={() => { setFile(null) }}>
                                                            🗑️
                                                        </button>
                                                    </div>
                                                )}
                                    ...
                                    <div className='flex flex-row mt-12 '>
                                        <button className='w-[124px] h-[34px] bg-purple-500 border rounded-md ml-12'
                                            onClick={() => { uploadLink ? handleEditSubmit(editId, subjectValue, link) : handleEditSubmit(editId, subjectValue, file) }}>
                                            <p className='text-white text-xs'>Save and Exit</p>
                                        </button>
                                        <button className='ml-3 w-[124px] h-[34px] bg-white border border-purple-500 rounded-md'
                                            onClick={() => {setEditModalOpen(false); setSubjectValue(""); setLink(""); setFile(null)}}>
                    ...
                    >
                        ...
                        <div className='flex flex-col'>
                            ...
                            <div className="mt-3">
                                ...
                                <div className='flex flex-row mt-12 '>
                                    <button className='w-[124px] h-[34px] bg-purple-500 border rounded-md ml-12'
                                        onClick={() => { uploadLink ? handleSubmit(subjectValue, link) : handleSubmit(subjectValue, file) }}>
                                        <p className='text-white text-xs'>Save and Exit</p>
                                    </button>
                                    <button className='ml-3 w-[124px] h-[34px] bg-white border border-purple-500 rounded-md'
                                        onClick={() => {setModalOpen(false); setSubjectValue(""); setLink(""); setFile(null)}}>
                                        <p className='text-purple-500 text-xs'>Exit</p>
                                    </button>
                                </div>
                            </div>
                ...
                <table className="border border-gray-300 rounded-lg border-separate border-spacing-0">
                    <thead className='rounded-lg'>
                        <tr className="bg-gray-200 rounded-lg">
                            <th className="p-2 border border-r border-gray-300 w-[500px]">Subject</th>
                            <th className="p-2 border border-r border-gray-300 w-[500px]">File/Link</th>
                            ...
                            <tr key={row.id} className={index % 2 === 0 ? "bg-gray-100" : "bg-white"}>
                                ...
                                <td className="p-2 border">
                                    {typeof row.file === 'string' ? (
                                        <a href={row.file} target="_blank" rel="noopener noreferrer" className="text-blue-600 underline">
                                            {row.file}
                                        </a>
                                    ) : (
                                        <p>{row.file.name}</p>
