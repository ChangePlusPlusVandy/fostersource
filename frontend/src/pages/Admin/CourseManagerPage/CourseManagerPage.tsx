	...
	const updateManagers = async (newManagerIds: string[]) => {
		try {
			...
			try {
				...
				console.log(
					"Operation might have succeeded despite error. Refreshing data..."
				);
			...
			setError(
				"Error updating managers, but changes might still be saved. Please check after refresh."
			);
		} finally {
			setIsLoading(false);
		}
