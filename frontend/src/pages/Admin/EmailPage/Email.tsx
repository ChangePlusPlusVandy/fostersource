...
export default function EmailPage() {
  ...
  const [emails, setEmails] = useState<Email[]>([
    ...Array(50).fill(null).map((_, i) => ({
      id: i + 1,
      subject: 'Details for Tomorrow #' + (i + 1) + ': Please READ: This is going to be AWESOME!',
      product: 'Avoiding Head Trauma and Early Exposure - Live Virtual (01/18/2025)',
              ...
              <button
                ...
                onClick={() => {
                  ...
                  setCurrentPage(prevPage => {
                    const totalPages = Math.ceil(emails.filter(e => !e.selected).length / itemsPerPage);
                ...
                <table className="min-w-full table-auto border-collapse">
                  <thead>
                    <tr>
                      ...
                      <th className="px-3 py-3 text-left text-sm font-bold text-black border-r">Subject</th>
                      <th className="px-3 py-3 text-left text-sm font-bold text-black border-r">Product</th>
                      <th className="px-3 py-3 text-left text-sm font-bold text-black border-r">Send Time</th>
                      <th className="px-3 py-3 text-center text-sm font-bold text-black">Actions</th>
                    </tr>
                  </thead>
                  <tbody>
                    {displayedEmails.map((email) => (
                      <tr key={email.id} className={`border-t odd:bg-[#f2f2f2]`}>
                        <td className="px-4 py-2 h-12">
                          <input
                            ...
                            onChange={() => {
                              setEmails(
                                emails.map((e) =>
                                  e.id === email.id ? { ...e, selected: !e.selected } : e
                                )
                              );
                            }}
                            className="w-5 h-5"
