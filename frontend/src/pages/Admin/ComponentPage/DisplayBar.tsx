const DisplayBar = ({ status }: { status: "webinar" | "survey" | "certificate" }) => {
  const getButtonStyle = (button: "webinar" | "survey" | "certificate") => {
    ...
    const colors = {
      ...
      survey: status === "survey" ? "#F79518" : status === "certificate" ? "#FBD9A5" : "#D9D9D9",
  ...
  return (
    <div className="mt-1" style={{ display: "flex", alignItems: "center", gap: "2px" }}>
      {/* Webinar Button */}
      <button
        style={{
