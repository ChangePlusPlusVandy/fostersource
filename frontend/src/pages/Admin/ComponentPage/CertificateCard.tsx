...
export default function CertificateCard({ workshop, prerequisites, setPrerequisites }: WorkshopProps) {
  const formatMenuItems = [
    { label: "None Selected", onClick: () => setPrerequisites((prev) => ({ ...prev, survey: "None Selected" })) },
    { label: "Survey", onClick: () => setPrerequisites((prev) => ({ ...prev, certificate: "Survey" })) }
  ];
    ...
    <div className="border rounded-lg shadow p-4 bg-white w-full last:mr-6 h-[98%] pb-2">
      <div>
        <div className="flex flex-row">
            <Vault />
            <h2 className="text-xl font-bold pl-2">{workshop?.name || "Certificate"}</h2>
        </div>
        ...
        <div className="mt-2 border p-3 rounded-lg border-black h-[350px] relative flex items-center flex-col">
            ...
            <div className="text-black text-sm font-medium space-y-2 w-full pt-7 pb-20 mb-24"><span></span></div>
