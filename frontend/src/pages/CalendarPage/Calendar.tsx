							...
							return (
								...
								>
									<div className="font-medium mb-1 flex items-center">
										<div
											className={`${isToday(day) ? "bg-orange-500 text-white w-6 h-6 rounded-full flex items-center justify-center" : ""}`}
										>
											{format(day, "d")}
										</div>
									</div>
									<div className="space-y-1">
										{events.map((event) => (
											...
											>
												...
												<span
													className="ml-1 overflow-hidden whitespace-nowrap text-ellipsis"
						...
						<div className="border-r bg-white sticky left-0 w-32">
							<div className="p-4 text-base">
								<div className="flex flex-col">
									...
									<div
										className={`mt-1 text-sm font-bold inline-flex ${
											isToday(currentDate)
				...
				{tooltip.visible && tooltip.event && (
					...
					>
						...
						<div className="mt-2 text-sm">
							<p>
								{tooltip.event.type.charAt(0).toUpperCase() +
									tooltip.event.type.slice(1)}
							</p>
							<p>
								{format(tooltip.event.date, "MM/dd/yyyy")} at{" "}
								{format(tooltip.event.date, "h:mm a")}
							</p>
							<br />
							<p>{tooltip.event.description}</p>
							<br />
							<p className="font-semibold">
								Hours earned: {tooltip.event.creditHours}
							</p>
						</div>
					</div>
				)}
