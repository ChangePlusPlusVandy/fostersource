...
describe('Survey Response Controller', () => {
  ...
  it('should return all survey responses', async () => {
    const questionResponse = await QuestionResponse.create({ question: 'Q1', answer: 'A1' });
  ...
  it('should create a survey response', async () => {
    const questionResponse = await QuestionResponse.create({ question: 'Q2', answer: 'A2' });

    const res = await request(app).post('/api/surveyResponses').send({
      userId: '123',
