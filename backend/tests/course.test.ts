...
describe("POST /api/courses", () => {
    it("should fail to create a new course if missing data", async () => {
        ...
        expect(res.body.message).toBe("Please provide className, isLive, creditCount, description, and thumbnailPath");
    });
    ...
    it("should create a new course if all required fields are provided", async () => {
        ...
        expect(res.body.data.description).toBe("This is a description for Course A");
...
describe("PUT /api/courses/:id", () => {
    ...
    it("should return 404 if course is not found", async () => {
        ...
        const res = await request(app).put(`/api/courses/${invalidCourseId}`).send({ className: "Course X" });
...
describe("DELETE /api/courses/:id", () => {
    ...
    it("should delete course and associated ratings and components", async () => {
        ...
        expect(res.statusCode).toBe(200);
        expect(res.body.message).toBe("Course and associated data deleted successfully.");
