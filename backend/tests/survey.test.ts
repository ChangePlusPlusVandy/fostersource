...
describe("POST /api/handouts", () => {
	...
	it("should create a new handout", async () => {
		...
		const res = await request(app).post("/api/handouts").send(newHandoutData);
	...
	it("should return 400 if required fields are missing", async () => {
		...
		const res = await request(app).post("/api/handouts").send(incompleteData);
...
describe("DELETE /api/handouts/:id", () => {
	...
	it("should delete an existing handout", async () => {
		...
		const res = await request(app).delete(`/api/handouts/${handoutData1._id}`);
	...
	it("should handle server errors during deletion", async () => {
		mockHandout.findByIdAndDelete.mockRejectedValue(
