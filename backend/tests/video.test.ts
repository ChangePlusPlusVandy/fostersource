...
describe('Video Controller - Get Videos', () => {
  it('should return all videos', async () => {
    await Video.create([{ title: 'Test Video 1', description: 'Desc 1', published: true }]);
  ...
  it('should filter videos by title', async () => {
    await Video.create([{ title: 'Test Video 2', description: 'Desc 2', published: false }]);

    const res = await request(app).get('/api/videos').query({ title: 'Test Video 2' });
    expect(res.status).toBe(200);
    expect(res.body.count).toBe(1);
...
describe('Video Controller - Create Video', () => {
  it('should create a new video', async () => {
    const res = await request(app)
      .post('/api/videos')
      .send({ title: 'New Video', description: 'New Description', published: true });
...
describe('Video Controller - Update Video', () => {
  it('should update an existing video', async () => {
    const video = await Video.create({ title: 'Old Title', description: 'Old Desc', published: false });

    const res = await request(app)
      .put(`/api/videos/${video._id}`)
      .send({ title: 'Updated Title', description: 'Updated Desc', published: true });
...
describe('Video Controller - Delete Video', () => {
  it('should delete an existing video', async () => {
    const video = await Video.create({ title: 'To be deleted', description: 'Temp Desc', published: false });
  ...
  it('should return 404 if video does not exist', async () => {
    const res = await request(app).delete(`/api/videos/60c72b1f4f1a2563dc5fa66b`);
    expect(res.status).toBe(404);
    expect(res.body.success).toBe(false);
