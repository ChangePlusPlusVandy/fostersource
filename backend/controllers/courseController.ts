...
export const getCourseById = async (
  ...
  try {
    ...
    console.log("[getCourseById] Requested course ID:", id);
    ...
    if (id) {
      ...
      if (!course) {
        console.log("[getCourseById] Course not found for ID:", id);
        res.status(404).json({
          success: false,
      ...
      console.log("[getCourseById] Course found:", course._id);
      console.log("[getCourseById] Course managers:", course.managers);
      res.status(200).json({
        success: true,
        data: course,
      });
    } else {
      console.log("[getCourseById] Invalid ID provided");
      res.status(401).json({
        success: false,
        message: "Invalid ID",
      });
    ...
    console.error("[getCourseById] Stack:", error.stack || "No stack trace available");
...
export const createCourse = async (
  ...
  res: Response
    ...
    console.error("[createCourse] Error message:", error.message || "No message available");
    console.error("[createCourse] Stack:", error.stack || "No stack trace available");
  ...
  try {
    ...
    if (updates.managers) {
      ...
      const validManagerIds = updates.managers.filter((managerId: string) => {
        ...
        if (!isValid) {
          console.warn(`[updateCourse] Invalid manager ID format: ${managerId}`);
        }
        return isValid;
      });
      
      if (validManagerIds.length !== updates.managers.length) {
        console.warn("[updateCourse] Some manager IDs were invalid and filtered out");
      }
      ...
      try {
        ...
        console.error("[updateCourse] Original managers data type:", typeof updates.managers);
    ...
    if (!updatedCourse) {
      ...
      res.status(404).json({ success: false, message: "Course entry not found" });
      return;
    }
    ...
    const error = err as ErrorWithDetails;
    console.error("[updateCourse] Error type:", error.constructor?.name || "Unknown");
    console.error("[updateCourse] Error message:", error.message || "No message available");
    console.error("[updateCourse] Stack:", error.stack || "No stack trace available");
    console.error("[updateCourse] Course ID:", req.params.id);
    console.error("[updateCourse] Update payload:", JSON.stringify(req.body, null, 2));
...
export const deleteCourse = async (
  ...
  try {
    ...
    console.error("[deleteCourse] Stack:", error.stack || "No stack trace available");
