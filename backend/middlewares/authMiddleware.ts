...
export const verifyToken = async (
  ...
  try {
    ...
    try {
      ...
      if (error instanceof jwt.TokenExpiredError && refreshToken) {
        try {
          ...
          const jwtPayload = jwt.verify(newTokens.accessToken, process.env.JWT_SECRET as string) as JWTPayload;
    ...
    if (error instanceof jwt.TokenExpiredError) {
      ...
      res.status(403).json({ message: (error as Error).message || "Authentication failed" });
    }
  }
};

async function validateTokens(req: Request, jwtPayload: JWTPayload, firebaseToken: string): Promise<void> {
...
async function refreshAuthToken(refreshToken: string) {
  try {
    ...
    const newRefreshToken = await admin.auth().createCustomToken(user.firebaseId);
